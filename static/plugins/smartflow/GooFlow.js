...
//初始化区域图的对象
	...
	height=height-headHeight-(property.haveHead? 5:5);
	this.$bgDiv.append("<div class='GooFlow_work' style='width:"+(width)+"px;height:"+(height+5)+"px;"+(property.haveHead? "":"margin-top:0px")+"'></div>");
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		//add by jjie
		var sname = "node_"+This.$max;
		if(This.$nowType=="start") sname="开始";
		if(This.$nowType=="end") sname="结束";
		
		This.addNode(This.$id+"_node_"+This.$max,{name:sname,left:X,top:Y,type:This.$nowType});
		This.$max++;
	  });
	  //划线或改线时用的绑定
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			defs.appendChild(GooFlow.prototype.getSvgMarker("arrow1",GooFlow.prototype.color.line||"#a8a8a8"));
		...
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
            ...
            form.active.editNodeWin("line",This.$lineData[this.id],this.id);
	...
	addNode:function(id,json){
		...
		var msg = (json.type.indexOf("msg_") == 0) ? " GooFlow_item_" + json.type : "";
		if(json.type.indexOf(" round")<0){
			if(!json.width||json.width<100)json.width=160;
            ...
            //自定义jjie
			/*
			if (json.type.indexOf("geo") == 0) {
			    if (!json.width || json.width < 100) json.width = 180;
			    if (!json.height || json.height < 24) json.height = 45;
			}
			*/

			this.$nodeDom[id]=$("<div class='GooFlow_item" + mark + msg + "' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td><td style='font-weight:bold'>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
			
		}
		else {
		    json.width = 24; json.height = 24;
		    this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + msg + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='0'><tr><td class='ico'><i class='ico_" + json.type + "'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span' style='font-weight:bold'>" + json.name + "</div></div>");
		}
	    if(GooFlow.prototype.color.node){
              if(json.type.indexOf(" mix")>-1){
                ...
                this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.node,"border-color":GooFlow.prototype.color.normalborder});
              }
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mouseleave",{inthis:this},function(e){
			...
			if(this.id==e.data.inthis.$focus){
        		$(this).css("border-color",GooFlow.prototype.color.line||"#a8a8a8");
			}else{
        		$(this).css("border-color",GooFlow.prototype.color.normalborder||"#a8a8a8");
			}
		});
		//绑定连线时确定初始点
			...
			if(lineStart&&!This.$mpTo.data("p")){
				...
				if(!This.$nodeData[this.id].marked){
					  ...
					  if(this.id!=This.$focus){
						$(this).css("border-color",GooFlow.prototype.color.normalborder);
					  }
					  else{
		...
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
            ...
            form.active.editNodeWin("area",null,null);
		...
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			...
			//节点双击弹出框 addby jjie
            form.active.editNodeWin("node",This.$nodeData[id],id);
			/*
			var t=getElCoordinate(This.$workArea[0]);
			...
			if(jq.prop("tagName")=="DIV"){
				...
				if(GooFlow.prototype.color.line){
					  if(this.$nodeData[this.$focus].marked){
						...
						jq.css("border-color",GooFlow.prototype.color.normalborder||"#a8a8a8");
					  }
				}
			}
			else{
				...
				if(GooFlow.prototype.useSVG!=""){
					if(!this.$lineData[this.$focus].marked){
						jq[0].childNodes[1].setAttribute("stroke",GooFlow.prototype.color.line||"#a8a8a8");
						jq[0].childNodes[1].setAttribute("marker-end","url(#arrow1)");
					}
				}
				else{
					if(!this.$lineData[this.$focus].marked)	jq[0].strokeColor=GooFlow.prototype.color.line||"#a8a8a8";
				}
	...
	focusItem:function(id,bool){
		...
		if(jq.prop("tagName")=="DIV"){
			...
			if(GooFlow.prototype.color.line){
        		jq.css("border-color",GooFlow.prototype.color.focusborder);
			}
			...
			if(GooFlow.prototype.useSVG!=""){
				jq[0].childNodes[1].setAttribute("stroke",GooFlow.prototype.color.focusborder||"#3892d3");
				jq[0].childNodes[1].setAttribute("marker-end","url(#arrow2)");
			}
			else jq[0].strokeColor=GooFlow.prototype.color.focusborder||"#3892d3";
			if(!this.$editable)	return;
			var x,y,from,to,n;
			if(GooFlow.prototype.useSVG!=""){
...
///////////以下为有关画线的方法
	...
	drawLine:function(id,sp,ep,mark,dash){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			if(mark){
				...
				path.setAttribute("stroke",GooFlow.prototype.color.line||"#a8a8a8");
			...
			else	line.strokeColor=GooFlow.prototype.color.line||"#a8a8a8";
			line.fillColor=GooFlow.prototype.color.line||"#a8a8a8";
		}
		return line;
	},
	//画一条只有两个中点的折线
	drawPoly:function(id,sp,m1,m2,ep,mark){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			else	poly.strokeColor=GooFlow.prototype.color.line||"#a8a8a8";
		}
		return poly;
	},
        ...
        jq.css("border-color",GooFlow.prototype.color.mark);
			...
			if(GooFlow.prototype.useSVG!=""){
				if(mark){
					...
					this.$nodeDom[id].childNodes[1].setAttribute("stroke",GooFlow.prototype.color.line||"#a8a8a8");
				...
				else	this.$nodeDom[id].strokeColor=GooFlow.prototype.color.line||"#a8a8a8"
			}
		}
		if(this.$undoStatck){
